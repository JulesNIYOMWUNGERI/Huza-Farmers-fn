...
const Auth_page = () => (
    <motion.div 
      className='bg-[#8f8f8f] w-screen h-auto py-[60px] flex justify-center items-center z-[3]'
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
    >
       <div className='w-[80%] h-auto py-[100px] bg-white rounded-xl flex flex-col items-center justify-center z-[2] relative overflow-hidden'>
         ...
         <div className={`${styles.flexCenter} ${styles.paddingX} flex-row flex-wrap gap-5 w-full z-[10]`}>
            <div className={`md:w-[40%] w-[100%] ${styles.paddingY} flex flex-col gap-10 bg-gray-50 ${styles.flexCenter} overflow-hidden`}>
                ...
                <Link to='/auth/CustomerAuth'>
                   <motion.button whileHover={{ scale: 1.1 }} type="button" className={`py-2 px-5 font-poppins font-medium text-[18px] text-primary bg-green-gradient rounded-[10px] outline-none cursor-pointer`}>
                       Regester now
                   </motion.button>
                </Link>
            </div>
         ...
         <div className='bg-green-500 h-[170px] w-[250px] z-[-1] absolute right-0 top-0'></div>
